...
export async function seedWalletCollection() {
  try {
    ...
    let testUser = await User.findOne({ walletAddress: sampleWalletAddress.toLowerCase() });
    if (!testUser) {
      testUser = new User({
    ...
    const sampleQuests = [
      {
        ...
        status: 'active',
        creator: (testUser._id as any).toString(),
        participants: [(testUser._id as any).toString()],
    ...
    for (const questData of sampleQuests) {
      const existingQuest = await Quest.findById(questData._id);
      if (!existingQuest) {
    ...
    const sampleStars = [
      {
        ...
        discoverer: (testUser._id as any).toString(),
        discoverers: [(testUser._id as any).toString()],
        metadata: {
        ...
        status: 'discovered',
        discoveredBy: (testUser._id as any).toString(),
    ...
    for (const starData of sampleStars) {
      const existingStar = await Star.findById(starData._id);
      if (!existingStar) {
    ...
    // Save NFTs and update user's collection
    const nftIds: string[] = [];
    for (const nftData of sampleNFTs) {
      ...
      if (!existingNFT) {
        ...
        await nft.save();
        nftIds.push((nft._id as any).toString());
      } else {
        nftIds.push((existingNFT._id as any).toString());
      }
    }

    // Update user with NFT references
