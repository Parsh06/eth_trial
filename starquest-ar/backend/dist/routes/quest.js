...
router.get('/', async (req, res) => {
    try {
        const { category, difficulty, location, radius = 10000, limit = 20, page = 1 } = req.query;
        ...
        const quests = await Quest_1.Quest.find(filter)
...
router.get('/:id', async (req, res) => {
    try {
        const quest = await Quest_1.Quest.findById(req.params.id)
            .populate('creator', 'username avatar level')
...
router.post('/', authenticateToken, async (req, res) => {
    try {
        const { title, description, difficulty, category, location, stars, rewards, requirements, maxParticipants, startDate, endDate, metadata } = req.body;
        if (!title || !description || !difficulty || !category || !location) {
            return res.status(400).json({ error: 'Missing required fields' });
        }
        const quest = new Quest_1.Quest({
            title,
            description,
            difficulty,
            category,
...
router.get('/user/my-quests', authenticateToken, async (req, res) => {
    try {
        ...
        const filter = { participants: req.user.userId };
        if (status !== 'all') {
