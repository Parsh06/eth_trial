...
async function seedLocationData() {
    try {
        ...
        await Star_1.Star.deleteMany({});
        console.log('🗑️ Cleared existing stars');
        const sampleQuest = await Quest_1.Quest.findOne({ name: "City Explorer" });
        let questId = sampleQuest?._id;
        if (!questId) {
            const newQuest = new Quest_1.Quest({
                name: "City Explorer",
        ...
        for (const location of sampleLocations) {
            const star = new Star_1.Star({
                ...
                quest: questId,
                metadata: arModels[location.type],
